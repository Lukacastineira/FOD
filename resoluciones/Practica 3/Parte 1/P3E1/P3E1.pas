...
program P3E1;
  ...
  procedure agregarEmpleadoAlFinal (var arc_logico:archivo);
    ...
    procedure leer(var arc_logico:archivo; var e:empleado);
    begin
        if (not EOF(arc_logico)) then
            read(arc_logico,e)
        else
            e.numEmple := valorAlto;
    end;

    procedure baja(var arc:archivo);
    var 
        numEmpleBaja:integer; e,ultEmple:empleado;
    begin
        ...
        leer(arc,e);
         ...
         while (e.numEmple <> valorAlto) and (e.numEmple <> numEmpleBaja) do
         begin
             leer(arc,e);
         end;

         if (e.numEmple = numEmpleBaja) then
         begin
             seek(arc,filePos(arc)-1);
             write(arc,ultEmple);
             seek(arc,fileSize(arc)-1);
             truncate(arc);
             writeln('Se elimino correctamente al empleado ', numEmpleBaja);
         end
         else
             writeln('No se encontro al empleado ', numEmpleBaja);
         close(arc);
    end; 
